export const siteCopy = {
  hero: {
    headline: "NuRock Entertainment",
    tagline:
      "NuRock Entertainment is a private teaching studio led by Holly NuRock. Through the NuRock Academy, artists learn real music production hands-on — arrangement, recording, mixing, and creative identity — building true skill from the ground up without relying on AI.",
    ctaPrimary: "Book a Session",
    ctaSecondary: "View Vault",
  },
  services: {
    title: "The Services",
    subtitle: "Repetition breeds perfection.",
    items: [
      {
        icon: "school",
        title: "Teaching",
        description:
          "One-on-one music production lessons inside a real studio. Holly teaches beat-making, arrangement, vocal production, mixing fundamentals, and the creative process — so you leave with skills you can use on every record.",
        href: "/artist-development",
      },
      {
        icon: "album",
        title: "Production",
        description:
          "Whether you're starting with a simple idea or a full sketch, we help you turn it into something that sounds like you—from bare bones to full-blown arrangements.",
      },
      {
        icon: "mic",
        title: "Recording",
        description:
          "Great mics, a room that feels right, and no pressure. We set things up so you can focus on the take and forget the rest.",
      },
    ],
  },
  about: {
    title: "The Teacher",
    titleAccent: "Teacher",
    body1:
      "Teaching is at the core of NuRock. Holly NuRock built the Academy so artists and producers can learn the craft the way he learned it — in the room, on real sessions, with hands-on guidance through every step of making a record.",
    body2:
      "From first beat to final mix, students learn arrangement, sound design, vocal production, mixing, and creative decision-making. No shortcuts. No AI crutches. Just real skill, real sound, and a clear path to release-ready music.",
    stat1: { value: "1:1", label: "Mentorship" },
    stat2: { value: "$150", label: "Per Hour Starting" },
    established: "Est. 2024",
  },
  cta: {
    headline: "Ready to work?",
    button: "Book Your Session Now",
  },
  footer: {
    tagline:
      "Leading the sonic revolution with artistic integrity and industrial precision. Your sound, our architecture.",
    connect: {
      email: "Hollynurock@nurockentertainment.com",
    },
    legal: "© 2024 NuRock Entertainment. All Rights Reserved.",
    links: [
      { label: "Privacy Policy", href: "#" },
      { label: "Terms of Art", href: "#" },
    ],
  },
  artistDevelopment: {
    title: "NuRock Academy",
    subtitle: "From raw talent to release-ready.",
    body: [
      "NuRock Academy is more than a production class. It is a full creative immersion into who you are as an artist, producer, writer, and storyteller.",
      "Led by Holly NuRock, the Academy is designed to help artists step into a world they may not have fully accessed yet, a space where sound, emotion, identity, and instinct come together. Through one-on-one mentorship, hands-on production, songwriting, and creative direction, Holly helps each artist uncover their unique frequency and translate it into a sound that feels true to them.",
      "As a Muse Producer, Holly’s gift is not just building records. It is bringing out the artist beneath the surface. He helps you discover your sonic palette, refine your creative identity, and turn raw emotion into release-ready music with purpose, depth, and direction.",
      "NuRock Academy is where artists stop copying what they hear and start creating from who they are.",
    ],
    pricing: {
      label: "Music Production Lessons",
      rate: "Starting @ $150/hr",
      minimum: "2 hour minimum",
    },
    cta: "Book a consultation",
  },
};
